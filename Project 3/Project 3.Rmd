...
#### Predict the SVD Recommender System on the test set
```{r}
svd_predict <- predict(svd_model, movie_test, n = 5)
```


Let's produce a list of the top 5 movie recommendations based on the SVD Recommender System.

```{r}
svd_reco_user_1 <- item_predict@items[[1]]
svd_movie_user_1 <- item_predict@itemLabels[svd_reco_user_1]
svd_movie_user_1 %>% kable(caption = "User 1 Recommendations: SVD") %>% kable_styling("striped", full_width = TRUE)
```
